...
import '../../widget/teacher_form.dart';
...
class _TeacherManagementState extends State<TeacherManagement> {
                              ...
                              children: [
                                Expanded(
                                  child: DataTable2(
                                    ...
                                    rows: pageItems.map((teacher) {
                                      return DataRow2(
                                        cells: [
                                          DataCell(Text(teacher.teacherId?.toString() ?? 'N/A')),
  ...
  void _showAddTeacherDialog(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (dialogCtx) => TeacherForm(
        onSubmit: (teacher) async {
          bool ok = false;
          String errorMessage = 'Thêm giảng viên thất bại';
          try {
            ok = await outerContext.read<AppController>().createTeacher(teacher);
          } catch (e) {
            errorMessage = e.toString().replaceFirst('Exception: ', '');
          }
          if (ok) {
            Navigator.of(dialogCtx).pop();
            _showSnack(outerContext, 'Thêm giảng viên thành công', true);
          } else {
    ...
    showDialog(
      ...
      builder: (dialogCtx) => TeacherForm(
        ...
        onSubmit: (updatedTeacher) async {
          ...
          try {
            ok = await outerContext.read<AppController>().updateTeacher(updatedTeacher);
          } catch (e) {
            errorMessage = e.toString().replaceFirst('Exception: ', '');
          }
          if (ok) {
            Navigator.of(dialogCtx).pop();
            _showSnack(outerContext, 'Cập nhật giảng viên thành công', true);
          } else {
            _showSnack(outerContext, errorMessage, false);
          }
        },
